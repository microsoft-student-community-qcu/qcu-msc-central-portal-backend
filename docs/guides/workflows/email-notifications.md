...
The layout uses a SaaS-style design: hidden preheader text, full-width org banner (600x150) atop a centered card with a subtle border and shadow, and a footer with the four social links (Facebook, Instagram, LinkedIn, TikTok) as embedded base64 PNG icons with visible labels, a `mailto:` contact link (`msc-qcu@outlook.com`), and a standard opt-out disclosure. All dynamic values are HTML-escaped via `esc()`/`escapeAttribute()`.
...
| 2026-08-03 | Restyled template to a SaaS-like layout: preheader, centered bordered card, and footer with embedded PNG social icons (FB/IG/LinkedIn/TikTok) with labels, `mailto:` contact (`msc-qcu@outlook.com`), and opt-out disclosure |
