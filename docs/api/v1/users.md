...
> **Safety net:** If this endpoint is never called (network timeout, page refresh, frontend bug), the **auto-link on sign-in** fallback reconnects the records automatically. Both `POST /api/v1/auth/student/sign-in` and `POST /api/v1/auth/admin/sign-in` run an idempotent `updateMany({ where: { email, userId: null }, data: { userId } })` when auth succeeds. No frontend change needed — the user just signs in again.
