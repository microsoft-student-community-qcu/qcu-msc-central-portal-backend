...
| Applicant | Prospective MSC member | Can access the application portal and submit resume/GitHub links. |
...
| Admin (Management & Dev) | Manages growth, onboarding, skill-building, and organizational documentation of human resources. | Has full access to the HR & Recruitment Pipeline: view/export sortable applicant lists, access portfolio links, mutate application statuses, and trigger automated or custom branded emails to candidates. |
| Admin (Logistics) | The engine of the organization, responsible for physical staging, event workflows, and technical backbones. | Has full access to Event Logistics & Check-In: create event entries (capacity/type), view attendee rosters, use the mobile-friendly QR scanner route (`/admin/events/scan`) to validate tickets, and perform manual check-in overrides. |
...
- **User Story 2 (The General Student):** As a non-member student, I want to register for an open workshop once general admission begins and receive a digital QR ticket to secure my slot.
...
- **Priority Verification:** During the "Priority Window" (the time between the `priorityStartDate` and `generalStartDate`), the UI must verify the user's status. Only users logged into the system with an active Member role can view and submit the registration form.
- **Capacity Deduction:** The registration route must accept both priority members and general students (once their respective windows open), seamlessly deducting from the single event capacity counter.
- **Ticket Dispatch:** Upon successful registration, the system generates a unique QR payload and emails it to the user.

**Edge Cases & Errors:**

- **Members-Only Block:** If a non-member attempts to register for an event flagged strictly as "Members-Only", the UI must permanently block the registration and prompt them to apply to the organization.
- **Premature General Access:** If a general student attempts to bypass the UI and hit the registration API endpoint during the "Priority Window," the backend must reject the request with a `403 Forbidden: General Admission has not started.` error.
